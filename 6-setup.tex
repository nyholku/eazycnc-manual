...
You can update the firmware in The Motor Controller screen, see .
...
EazyCNC comes with all the firmware versions that have been released at the time of
the EazyCNC release and those versions are listed in this popup menu.
...
If you want to use a separate firmware file select the 'From file...' 
...
This dialog box will disappear after the update is complete. A success/fail
...
If update was successful the firmware version should show up in the Status Display
...
displayed in the Toolpath display panel in the main screen.
...
tool table for the tool selected by the G-code program is used in displaying the path.
...
The color of the tool in the Toolpath display can be specified as well as the size of 
...
The size and colors used to display the  axis grids in the Toolpath display can be specified here.
...
Whenever the spindle has been turned ON with M3 or M4 G-code TOAD will
...
to be displayed and to protect the spindle EazyCNC will not execute a G-code file which
...
provides the most 'stiff' setup but means that motors will have
...
the TOAD board is 1 or 0 when home switch / reference switch is
active. 
...
your limit switch is set to activate in the exact middle of that 
...
slow down movement whenever you move the Z-axis you can use
thew scaler. 
...
When motors are paralleled they perform the exact same movements, except when you press the HOME button. 
...
Note that when motors are paralleled then all the settings for both motors are taken the
...
coordinates so it will lose track of the position
if it is turned off).
...
section~\ref{sec:test-panel}, to help you to 
...
See section~\ref{sec:test-panel}) on how to do this for
...
If you see a lot of large erroneous arcs in the tool path graphics panel when 
...
the tool to move in a straigth line until it has 'cleared' the corner and the move
...
away from the potentially sharp and narrow corner which tends to burn if the torch lingers
too long around the corner.

\subsubsection{Use round join -radio-button}
...
\subsubsection{Use miter join -radio-button}

If this is selected then tool will move  in a straigth line until it has 'cleared' the corner and the move
...
If 'N-axis auto zero'  checkbox  is ticked then that axis DRO will be automatically
zeroed when you hit the RUN button if the machine was in STOPed state.

The DRO will NOT be zeroed when you hit the RUN button when the system
is in HOLD state.
...
As mentioned the ',' (comma) can be both decimal separator and list item
...
The purpose of the shutdown script is to allow you to customize the
...
to the operating system provided because it is optimised For
...
\section{Test screen}
\label{sec:test-panel}


The Test screen 
allows you to test all the inputs and output of your TOAD boars as well
as test or find out the limits of your motors, \screencaps{mach-setup-view-Test}{0.5}{The Test screen.}
...
everything is, well, simulated).

\subsection{Inputs -panel}

This panel shows in real-time the state of  all the digital inputs from the TOAD board.
...
To exercise the outputs you need to first activate the orange OVERRIDE button. 
...
When you click the 'TEST button the test movement is performed as follows.
