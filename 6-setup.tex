...
Windows 10 & \PVerb{C:\Users\username} \\
...
If you are not runnig EazyCNC under Linux you can skip this section.
...
Alternatively following one-liner should accomplist setting up the rules for you:
...
\section{Selecting Motor Controller Type} 
\label{sec:selecting-motor-controller-type}

EazyCNC supports multiple different types of motor control hardware with different
capabilities and features. 

It is important that the Motor Controller selected in EazyCNC matches the hardware and especially
firmware that you are using.

Each motor controller has different configuration needs so before you can set up
the rest of the machine correctly you need to select the correct motor controller type.

To select the motor controller go to 'Controller' screen,  
see \screencap{mach-setup-view-Controller-TOAD4+}{The Motor Controller Type selection setup screen}.

Note that when you change the motor controller type you need to both SAVE and re-start EazyCNC
for the changes to take effect.

When you change the motor controller type many of the Axis Setup and Mach Setup screen
change subtly. 

\section{Updating Motor Controller Firmware } 
\label{sec:updating-motor-controller-firmware}

If you are using TOAD4+ (2.n.m) firmware NEVER update the firmware using a ICSP programmer or PICKit!

You can upate the firmware in The Motor Controller Type selection screen, figure~\ref{fig:mach-setup-view-Controller-TOAD4+}.

To update the firmware do as follows.

Connect your TOAD4+ hardware to a USB port and power up the controller. 
...
If the firmware update is interrupted you may have to force the TOAD4+ board
into the firmware update mode by removing the jumper between pins 4 and 5 of J6.
...
If this checkbox is ticked then all communication is between EazyCNC and TOAD4+
...
To do that go the the 'User Interface' setup screen 
\screencap{mach-setup-view-Screen}{The User Interface setup screen} 
...
The TOAD4/ board has outputs to control a Variable Frequency Drive (VFD) for
machine spindle control, an output to control a coolant pump/valve and an input for touch probe.

To setup and/or test your inputs and outputs go to the Input/Ouput setup screen,
see \screencap{mach-setup-view-IO}{The Input/Ouput setup screen}.

This screen also contains buttons etc to manually control the outputs for testing purposes.

\subsection{Probe Input -panel}

\subsubsection{Polarity -popup menu}

This popup allows you to set which probe input state (1 or 0) is interpreted as active.
Active means that EazyCNC interpretes the that the touch probe is touching something.
...
\subsection{SPEED INPUT -panel}

\subsubsection{ADC Value -entry field}

This entry field shows the digitized ADC (Analog to Digital Converter)
value of the TOAD4 SPEED input signal.

You can use this to check the operation of the input signal.

\subsection{Spindle Speed -panel}
...
should enter into EazyCNC here. 
...
\subsection{Spindle Outputs -panel}

\subsubsection{Automatic SPINDLE OFF -checkbox}

If this checkbox is ticked then the spindle is automatically turned OFF when the system moves 
from RUN state to HOLD or STOP state. It is still possible to use the SPINDLE button manually 
and indeed it is necessary to manually turn ON the SPINDLE if you just paused the machining
with the HOLD button; the spindle is NOT turned ON automatically.

This feature is especially very handy with plasma cutting where it is mandatory to turn off
the torch when the cutting is paused, otherwise the plasma jet will eat a big hole in the workpiece.


\subsubsection{FORWARD/REVERSE -controls}

These controls are for testing and trouble shooting the spindle drive setup.

They each have a 'LED' that indicates the output state, weather it is controlled by
the Machine Control buttons at the bottom of the screen, G-code or the Manual Override ON/OFF
buttons. 

If the 'Enable manual override' -checkbox is ticked then the output state is controlled by the
ON/OFF buttons. Take care when using these as it is easy to damage your equipment and hurt
yourself with the manual overrides. Of course it easy to do that with the 'normal' controls as well.


\subsubsection{SPEED -entry field}
 
If the 'Enable manual override' -checkbox is ticked then the value in this entry field
directly controls the analog SPINDLE SPEED signal. 

The SPINDLE SPEED signal is ratiometric meaning the output voltage is a percentage of the
voltage fed into the +10V REF input.

Entering 0 to this entry field sets the SPINDLE SPEED signal to 0 \% of the reference voltage.

Entering 100 to this entry field sets the SPINDLE SPEED signal (close to) 100\% of the reference voltage.

\subsection{Coolant Outputs -panel}

\subsubsection{Automatic COOLANT OFF -checkbox}

If this checkbox is ticked then the coolant is automatically turned OFF when the system moves 
from RUN state to HOLD or STOP state. It is still possible to use the COOLANT button manually 
and indeed it is necessary to manually turn ON the coolant if you just paused the machining
with the HOLD button; the coolant is NOT turned ON automatically.

\subsubsection{COOLANT -controls}

These controls are for testing and trouble shooting the coolant control setup.

They each have a 'LED' that indicates the output state, weather it is controlled by
the Machine Control buttons at the bottom of the screen, G-code or the Manual Override ON/OFF
buttons. 

\subsection{Enable manual override -checkbox}

If this checkbox is 'ticked' then the ON/OFF buttons and the SPEED entry field are enabled
and the Mach Control buttons at the bottom of the screen nor G-code no longer control the
outputs, instead they are controlled by afore mentioned buttons and entry field.

\emph{Enabling the manual override can be dangerous because now you can directly control the machinery that
is connected to those outputs!}

If you click the FORWARD button and you have your VFD on it is likely that the spindle on
your machine will start to rotate or the plasma torch turns on! Beware!
...
see \screencap{axis-setup-view-axis-X}{Axis Setup screen}
...
\subsection{Zero DRO on start of RUN -checkbox}


If this checkbox is ticked then this axis DRO will be automatically
...
The maximum theoretical step rate is about 100 kHz, for jitter
and other reasons the maximum recommended pulse rate is about
$\frac{1}{5}$ of that, say 10000 pulses/sec. You need to divide
...
TOAD4+ allows you to select for each axis which physical motor is used to drive the axis.
...
TOAD4+ allows you to select for each axis a second physical motor that is driven when that axis moves.
...
Homing is performed indipendently for each motor so that the system will align itself correctly based
...
are definec by where you HOME-switch is located and 
...
\screencap{mach-setup-view-Motion}{The Motion Limits screen.}
 screen 
...
You can change them in the G-code screen, 
see \screencap{mach-setup-view-G-code}{The G-code screen.}
...
see \screencap{mach-setup-view-Shortcuts}{The Shortcuts setup screen.}
...
see \screencap{mach-setup-view-Info}{The System Info screen}
...
\section{Export/Import screen}
\label{sec:export-import-setup}

The Tool Setup screen allows you to save and load the tool setup to/from a text file, 
see section \ref{sec:tool-setup}

In the 
\screencap{mach-setup-view-ExportImport}{The Export/Import screen.}
you can setup details of text file format.

\subsection{File format -popup menu}
...
\subsection{Number format -entry field}
...
\subsection{List item Separator -popup menu}
