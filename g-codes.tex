...
avoid the differences between machine. Different interpretation for
...
In this document an asterisk, '\texttt{~}' after a word is used to denote a
...
optional or not is explained in the text. A '\texttt{*}' is used to 
...
the '\texttt{L}' word followed one and up to five digits.
...
The highest precedence group only has the the exponentiation or power operator '\texttt{**}'.
...
i.e. they storage locations for values which can be used in place
...
that connect, one arc is less than 180\degree and the other is
larger.
...
\label{tab:order-of-transformations} 
  Apply G51 Scaling \\
  If in G91 mode interpret as incremental coordinates \\
...
the scaling factor the given axes. The scale factors for 
...
%----------------------------------------------------
\subsection{Temporary coordinate system offsets -- G52,G92,G92.1,G92.2,G92.3 codes}
...
\paragraph{\texttt{92 X~ Y~ Z~ A~ B~ C~}} sets the temporary
coordinate offsets for the given axes so that the current
tool position has the given coordinates. 

\paragraph{\texttt{92.1}} saves the temporary offsets to 
G-code parameters 5211..5216. 

\paragraph{\texttt{92.2}} clears the temporary offsets.

\paragraph{\texttt{92.3}} restores the temporary offsets from
the parameters 5211..5216.



\paragraph{\texttt{92.1 X~ Y~ Z~ A~ B~ C~}}
%----------------------------------------------------
...
a circular path in the active plane which is one of the the main coordinate planes XY,XZ or YZ
plane.
...
cutter and facing in the direction of the travel.
...
% all this text should be, noy in G-code reference section.
...
this mode the the IJK words specify the center coordinates
...
%----------------------------------------------------
\subsection{Call subroutine -- M98/M99}
\label{gcode-M98}
